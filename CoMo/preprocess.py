...
def construct_neighbor_graph(adata_omics1, adata_omics2, datatype='SPOTS', 
                             n_neighbors=3, n_contrast_neighbors=5): 
    """
    ...
    # dense_adj = construct_dense_adj(cell_position_omics1, n_neighbors=3)
    ...
    # contrastive graph
    cell_position_omics1 = adata_omics1.obsm['spatial']
    adj_contrast = construct_graph_by_coordinate(cell_position_omics1, n_neighbors=n_contrast_neighbors)
    adata_omics1.uns['adj_contrast'] = adj_contrast
...
def construct_dense_adj(cell_position, n_neighbors=3):
    """Constructing spatial neighbor graph and return dense adjacency matrix."""
    nbrs = NearestNeighbors(n_neighbors=n_neighbors + 1).fit(cell_position)
    _, indices = nbrs.kneighbors(cell_position)
    
    # 获取目标节点索引（x）和邻居索引（y）
    x = indices[:, 0].repeat(n_neighbors)
    y = indices[:, 1:].flatten()  # 排除自身，只保留邻居
    
    # 构建稠密邻接矩阵
    N = cell_position.shape[0]
    dense_adj = np.zeros((N, N), dtype=np.float32)
    dense_adj[x, y] = 1.0  # 邻接关系设为1
...
def adjacent_matrix_preprocessing(adata_omics1, adata_omics2):
    ...
    # contrastive adj
    adj_contrastive = adata_omics1.uns['adj_contrast']
    adj_contrastive = transform_adjacent_matrix(adj_contrastive)
    adj_contrastive = adj_contrastive.toarray()
    adj_contrastive = adj_contrastive + adj_contrastive.T
    adj_contrastive = np.where(adj_contrastive>1, 1, adj_contrastive)
    adj_contrastive = preprocess_graph(adj_contrastive)
