...
def refine_label(adata, n_neigh_refine=5, key='mclust'):
    new_type = []
    ...
    nbrs = sklearn.neighbors.NearestNeighbors(n_neighbors=n_neigh_refine).fit(coor)  # 使用k近邻
    _, indices = nbrs.kneighbors(coor)  # 获取k个最近邻的距离和索引
...
def clustering(adata, n_clusters=7, key='emb', add_key='CoMo', 
               method='mclust', start=0.1, end=3.0, increment=0.01, 
               use_pca=False, n_comps=20, 
               refinement=True, n_neigh_refine=5, refine_key='refine_CoMo'):
    """\
    ...
    if refinement:
        new_type = refine_label(adata, n_neigh_refine=n_neigh_refine, key=add_key)
        adata.obs[refine_key] = new_type
        adata.obs[refine_key] = adata.obs[refine_key].astype('int')
        adata.obs[refine_key] = adata.obs[refine_key].astype('category')
...
def plot_weight_value(alpha, label, modality1='mRNA', modality2='protein'):
  ...
  df.columns = ['label_CoMo', 'Modality', 'Weight value']
  ax = sns.violinplot(data=df, x='label_CoMo', y='Weight value', hue="Modality",
